...
import { fetchTiempoPromedioPrestamo } from "../../services/reporteService";
import AuthContext from "../../context/auth/AuthContext";
import ReporteTiempoPromedioPrestamo from "../../components/reporte/ReporteTiempoPromedioPrestamo";
import {
  MySwal,
  showErrorAlert,
  showLoadingAlert,
...
const ReporteTiempoPromedioPrestamoPage = () => {
  ...
  const loadReporteTiempoPromedioPrestamo = async (year1, year2) => {
    ...
    try {
      ...
      const reporteData = await fetchTiempoPromedioPrestamo(year1, year2, token.toString());
      MySwal.close();
      setReportData(reporteData);
    } catch (error) {
      MySwal.close();
      console.log(
        "Error al intentar obtener el reporte de tiempo promedio de préstamo: ",
        error
      );
  ...
  return (
    <>
      ...
      {reportData && year1 && year2 && ( // Ensure year1 and year2 are defined before passing
        <ReporteTiempoPromedioPrestamo reportData={reportData} year1={year1} year2={year2} />
      )}
    </>
  );
};

export default ReporteTiempoPromedioPrestamoPage;
